...
select ug.group_id from user_login_security_group as ug, user_login as u
where ug.user_login_id = u.user_login_id and u.user_login_id = 'admin';


--lấy ra các permission của user-login admin
select gp.permission_id from security_group_permission as gp where
group_id in (select group_id from user_login_security_group where user_login_id = 'admin');
...
select gp.group_id from security_group_permission as gp
...
select ug.user_login_id from user_login_security_group as ug where ug.group_id in
(select gp.group_id from security_group_permission as gp
where gp.permission_id in (select a.permission_id from application as a where a.application_id = 'MENU_PRODUCT_CREATE'));

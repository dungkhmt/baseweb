...
public class CBLSSolver {
    ...
    public CBLSSolver(int n, int m, HashSet[] D, boolean[][] conflict, double[] hourClass){
        ...
        for(int i = 0; i < m; i++) classesAssigned2Teacher[i] = new HashSet<Integer>();
    }
    ...
    private HashSet<Integer> initSolution(){
        ...
        for(int i = 0; i < n; i++) teacherAssigned2Class[i] = -1;// init NULL
        while(cand.size() > 0){
            ...
            for(int t: D[sel_i]){
                boolean ok = true;
                for(int j: assigned)if(conflict[sel_i][j] && teacherAssigned2Class[j] == t){
                    ok = false; break;
                }
                if(!ok) continue;
                if(load[t]  < minLoad){
    ...
    private boolean canAssignClass2Teacher(int i, int t){
        for(int j: classesAssigned2Teacher[t])if(conflict[i][j]) return false;
        return true;
    }
    ...
    private HashSet<Integer> getConflictClasses(int i, int t){
        ...
        for(int j: classesAssigned2Teacher[t])if(conflict[i][j]) s.add(j);
        return s;
    }
    class Move2Path{
        int i;
        ...
        public Move2Path(int i, int ti, int j, int tj){
            this.i = i; this.ti = ti; this.j = j; this.tj = tj;
        }
    }
    ...
    private boolean  findMultiMoves(int cls){
        ...
        for(int t: D[cls]){
            boolean ok = true;
            for(int j: classesAssigned2Teacher[t]) if(conflict[cls][j]){
                HashSet<Integer> Tj = getPossibleNewTeachers4Class(j);
                if(Tj.size() == 0){ ok = false; break;}
            }
            if(ok){
                int dt = 0;
        ...
        for(int i = 0;i < sorted_list.length-1; i++){
            for(int j = i+1; j < sorted_list.length; j++){
                if(hourClass[i] < hourClass[j]){
                    int tmp = sorted_list[i]; sorted_list[i] = sorted_list[j]; sorted_list[j] = tmp;
                }
            }
        }
        ...
        for(int i : notAssigned){
            if(D[i].size() > 0){
                for(int t: D[i]){
                    ...
                    for(int j: classesAssigned2Teacher[t])if(conflict[i][j]){
                        System.out.print(j + " ");
                    }
                    System.out.println();
                }

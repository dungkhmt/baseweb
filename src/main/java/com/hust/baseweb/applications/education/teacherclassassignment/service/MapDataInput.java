...
public class MapDataInput {
    ...
    public void genRandom(int n, int m, boolean opt){
        ...
        for(int j = 0; j < m; j++){
            for(int i1 = 0; i1 < n; i1++)if(x[i1] == j){
                for(int i2 = 0; i2 < n; i2++) if(x[i2] == j){
                    conflict[i1][i2] = false;
                }
            }
        }
        for(int i = 0; i < n; i++) conflict[i][i] = false;

        D = new HashSet[n];
        for(int i = 0; i < n; i++) D[i] = new HashSet<Integer>();
    ...
    public void loadDataFromPlanFile(String filename){
        try{
            ...
            for(int i = 0; i < n; i++){
                for(int j = 0; j < n; j++){
                    int x = in.nextInt();
                    if(x == 0) conflict[i][j] = false;
                    else conflict[i][j] = true;
                }
            }
            in.close();
        ...
        try{
                ...
                for(int i1 = 0; i1 < n; i1++) if(x[i1] == j){
                    for(int i2 = 0; i2 < n; i2++)if(x[i2] == j){
                        if(conflict[i1][i2] && i1 != i2){
                            ok = false;
                            System.out.println("FAILED: conflict i1 = " + i1 + " i2 = " + i2 + " assign to " + j);
                        }
                    }
                }
            }
            for(int j = 0; j < m; j++){
                ...
                if(L > maxHourTeacher[j]){
                    ok = false;
                    System.out.println("FAILED: teacher " + j + " has maxLoad = " + maxHourTeacher[j] + " but load = " + L);
                }
            }
            if(ok){

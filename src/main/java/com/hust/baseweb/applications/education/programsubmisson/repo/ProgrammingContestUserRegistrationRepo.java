...
@Repository
public interface ProgrammingContestUserRegistrationRepo
    extends JpaRepository<ProgrammingContestUserRegistration, CompositeProgrammingContestUserRegistrationId> {
    ...
    @Transactional
    @Modifying
    @Query(value = "update programming_contest_user_registration as p set p.status_id = ?3 where p.contest_id = ?1 and p.user_login_id = ?2",
           nativeQuery = true)
